...
class App extends Component {
  ...
  constructor(props) {
	...
	callAPI() {
    Axios.post("https://mysqlp2p.herokuapp.com0/withdrawal/getWithdrawals", {
    ...
    Axios.post("https://mysqlp2p.herokuapp.com0/account_stats/balance", {
        email: localStorage.getItem('emailReg'),
    }).then((response) => {
  ...
  SendOTP() {
    Axios.post(`https://mysqlp2p.herokuapp.com0/SendOTP/CommonOTP`, {
      email : localStorage.getItem('emailReg'),
    }).then(res => {
      if(res.data){
        this.setState({
  ...
  handleButtonClickedWithdrawed() {
    ...
    if(this.state.withdrawal_amount <= 0 || this.state.withdrawal_amount > this.state.current_balance ){
      ...
      if(parseInt(this.state.entered_otp) === this.state.original_otp){
        Axios.post(`https://mysqlp2p.herokuapp.com0/withdrawal/withdraw`, {
          email : localStorage.getItem('emailReg'),
